...
*Note : the factory will throw an exception by default if your parent class implements unsupported commands. You can change this behaviour by using the second parameter when you build the mock `$factory->getAdapterClass('My\Redis\Library', true)`. The exception will then thrown only when the command is called.*
