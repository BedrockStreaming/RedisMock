...
class RedisMock extends test
{
    public function testSetGetDelExists()
    {
        ...
        $this->assert->boolean($redisMock->exists('test'))->isFalse()->variable($redisMock->get('test'))->isNull()->integer($redisMock->del('test'))->isEqualTo(0)->string($redisMock->set('test',
            'something'))->isEqualTo(
            'OK'
        )->string(
            $redisMock->type('test')
        ...
        sleep(3);
        $this->assert->variable($redisMock->get('test3'))->isNull()->variable($redisMock->get('test4'))->isNull()->string($redisMock->set('test',
            'something', 1))->isEqualTo('OK')->string(
            $redisMock->type('test')
        )->isEqualTo('string');
        sleep(2);
    ...
    public function testZRange()
    {
        ...
        )->array($redisMock->zrange('test', 8, 2))->isEmpty()->array($redisMock->zrange('test', -1,
            2))->isEmpty()->array($redisMock->zrange('test', -3, 4))->isEqualTo(
            array(
                'test3',
                'test2',
            )
    ...
    public function testHSetHMSetHGetHDelHExistsHGetAll()
    {
        ...
        $this->assert->string($redisMock->set('test', 'something'))->isEqualTo('OK')->variable(
            ...
            'none'
        )->variable($redisMock->hget('test',
            'test1'))->isNull()->array($redisMock->hgetall('test'))->isEmpty()->integer($redisMock->hexists('test',
            'test1'))->isEqualTo(0)->integer(
            $redisMock->hset('test', 'test1', 'something')
