...
class RedisMock extends atoum
{
    ...
    {
        $redisMock = new Redis();
        $redisMock->zadd('myKey', 1, 'a1');
        $redisMock->zadd('myKey', 2, 'b1');
        $redisMock->zadd('myKey', 3, 'c1');
        $redisMock->zadd('myKey', 4, 'd1');
        ...
        $this->assert
            ->array($redisMock->zscan('myKey', 0, ['MATCH' => '*', 'COUNT' => 10]))
            ->isEqualTo([0 => 0, 1 => [0 => 'a1', 1 => 'b1', 2 => 'c1', 3 => 'd1']]);

        // It must return only the matched value
        $this->assert
            ->array($redisMock->zscan('myKey', 0, ['MATCH' => 'c*', 'COUNT' => 10]))
            ->isEqualTo([0 => 0, [0 => 'c1']]);

        // It must return all of the values based on the match of *1
        $this->assert
            ->array($redisMock->zscan('myKey', 0, ['MATCH' => '*1', 'COUNT' => 10]))
            ->isEqualTo([0 => 0, 1 => [0 => 'a1', 1 => 'b1', 2 => 'c1', 3 => 'd1']]);

        // It must return two values, starting cursor after the first value of the list.
        $this->assert
            ->array($redisMock->zscan('myKey', 1, ['COUNT' => 2]))
            ->isEqualTo([3, [0 => 'b1', 1 => 'c1']]);
        ...
        $this->assert
            ->array($redisMock->zscan('myKey', 4, ['MATCH' => '*', 'COUNT' => 10]))
            ->isEqualTo([0 => 0, 1 => []]);
        ...
        // It must return no values, as the key is expired.
        $this->assert
            ->array($redisMock->zscan('myKey', 0, ['COUNT' => 2]))
            ->isEqualTo([0, []]);
